...
import { Loader2, AlertCircle, Package, CheckCircle, ChevronUp, ChevronDown, Grid, List, Search } from 'lucide-react';
...
const PendingDelivery: React.FC = () => {
  ...
  const { customRequests, loading, error, markAsDelivered, fetchCustomRequests } = useCustomRequests();
  
  // Filter for completed status (ready for delivery)
  const completedCustoms = customRequests.filter(c => {
  ...
  const getSortedCustoms = (customs: typeof completedCustoms) => {
    if (!sortField) {
  ...
  const handleMarkAsDelivered = async (customId: string) => {
    const { error } = await markAsDelivered(customId);
    if (error) {
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            <span className="text-lg text-gray-600">
              {completedCustoms.length} of {customRequests.filter(c => c.status === 'completed').length} ready for delivery
            ...
            {searchTerm ? (
              <>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  No completed requests match "{searchTerm}". Try adjusting your search.
                </p>
                <div className="mt-6">
                  <button
              ...
              <>
                ...
                <h3 className="mt-2 text-sm font-semibold text-gray-900">All delivered!</h3>
                <p className="mt-1 text-sm text-gray-500">No completed customs waiting for delivery.</p>
              </>
            )}
          </div>
        ) : (
          <>
            {viewMode === 'cards' ? (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {getSortedCustoms(completedCustoms).map((custom) => (
                  <div key={custom.id} className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200 flex justify-end space-x-2">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleMarkAsDelivered(custom.id);
                        }}
