...
import { Loader2, AlertCircle, Clock, CheckCircle, XCircle, ChevronUp, ChevronDown, Grid, List, Search } from 'lucide-react';
...
const PendingTeamApproval: React.FC = () => {
  ...
  const { customRequests, loading, error, approveByTeam, denyByTeam, fetchCustomRequests } = useCustomRequests();
  const { teamMember } = useAuth();
  
  // Filter for pending team approval status
  const pendingCustoms = customRequests.filter(c => {
  ...
  const getSortedCustoms = (customs: typeof pendingCustoms) => {
    if (!sortField) {
  ...
  const handleApprovalClick = (custom: any) => {
    setSelectedCustom(custom);
  ...
  const handleApprove = async (customId: string) => {
    if (!teamMember) return { error: 'No team member found' };
    
    const { error } = await approveByTeam(customId, teamMember.id);
    return { error };
  };

  const handleDeny = async (customId: string) => {
    const { error } = await denyByTeam(customId);
    return { error };
  };
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            <span className="text-lg text-gray-600">
              {pendingCustoms.length} of {customRequests.filter(c => c.status === 'pending').length} pending approval
            ...
            {searchTerm ? (
              <>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  No pending requests match "{searchTerm}". Try adjusting your search.
                </p>
                <div className="mt-6">
                  <button
              ...
              <>
                ...
                <p className="mt-1 text-sm text-gray-500">No custom requests pending team approval.</p>
              </>
            )}
          </div>
        ) : (
          <>
            {viewMode === 'cards' ? (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {getSortedCustoms(pendingCustoms).map((custom) => (
...
export default PendingTeamApproval;