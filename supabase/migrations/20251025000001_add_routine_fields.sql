-- Add weekday_routine and weekend_routine columns to client_questionnaire table
-- These fields store descriptions of typical daily routines

ALTER TABLE client_questionnaire 
ADD COLUMN IF NOT EXISTS weekday_routine TEXT,
ADD COLUMN IF NOT EXISTS weekend_routine TEXT;

-- Add comments to document the column purposes
COMMENT ON COLUMN client_questionnaire.weekday_routine IS 'Description of an average weekday for the client';
COMMENT ON COLUMN client_questionnaire.weekend_routine IS 'Description of an average weekend day for the client';

-- Drop and recreate the upsert_client_questionnaire function to include the new fields
...
BEGIN
  ...
  VALUES (
    ...
    (p_questionnaire_data->>'weekdayRoutine')::TEXT,
    (p_questionnaire_data->>'weekendRoutine')::TEXT,
    NOW()
  )
  ...
  DO UPDATE SET
    ...
    weekend_routine = EXCLUDED.weekend_routine,
    updated_at = NOW();
END;
$$;

-- Drop and recreate the client_complete_profile view to include the new fields
